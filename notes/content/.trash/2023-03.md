...
TASK
FROM "Agenda/Daily"
WHERE !completed AND file.day.month = date("2023-03-01").month AND !contains(tags, "#habit") AND !rescheduled
GROUP BY file.name
```
...
## ⏳ focus time
```toggl
SUMMARY FROM 2023-03-01 TO TODAY
```
...
```dataview
CALENDAR file.mtime
WHERE !contains(file.name, "2023-03")
```